			...
			if (duration >= milliseconds) {
				...
				if (channel->openInfo->openAttempts == 0) {
					...
					if (DEVICES_EMPTY)
						...
						MOS_ERROR(iop, EPHIDGET_TIMEOUT, "No matching devices were found to open. Make sure your device is attached.");
				}
...
#define ALC_GEN_ERROR_ARGS device, PRC_ARGS(res)
		...
		if (channel->Error) {
			if (device->lastOpenErrstr != NULL && strlen(device->lastOpenErrstr) > 0)
				...
				FIRE_ERROR(channel, EEPHIDGET_FAILURE, ALC_BUSY_ERROR_STR, ALC_BUSY_ERROR_ARGS);
			else
